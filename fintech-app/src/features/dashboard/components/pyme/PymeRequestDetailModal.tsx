...
import { useAmlChecks } from "../../hooks/useAmlChecks";
import {
  STATUS_APPROVED,
  STATUS_PENDING,
  STATUS_ONREVIEW,
...
export const PymeRequestDetailModal: React.FC<PymeRequestDetailModalProps> = ({ isOpen, onClose, request }) => {
  ...
  const status = statusDisplay[request.status] || { label: request.status, color: 'bg-gray-100 text-gray-800' };
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl transform transition-all duration-300" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4 bg-gray-50 rounded-b-2xl flex justify-end">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">Cerrar</button>
        </div>
      </div>
    </div>
  );
